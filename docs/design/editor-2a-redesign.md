...
1. 块级快照 undo（每操作快照受影响块，max depth 1000）vs 操作日志——大文档性能与正确性取舍？
2. 中文 IME：text() 事件直接插块的方案在组合态（composition）下的生命周期是否要在 2a 处理完整，还是 2b 补？
3. code_block 内 Enter 是否拆块（普通块拆，代码块保留换行符单块）？
4. 可视区裁剪 + 块缓存失效策略：编辑时只重建受影响块（段落级增量），结构性编辑（拆/合块）全量重排——可接受？
